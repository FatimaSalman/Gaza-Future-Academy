    ...
    export function Library() {
      ...
      return (
        <div className="flex flex-col gap-8">
          ...
          {/* لوحة الفلاتر — direction: rtl يكفي بدون flex-row-reverse */}
          {showFilters && (
            <div className={cn(
              ...
              <div className="space-y-3">
                ...
                {/* ✅ بدون flex-row-reverse — direction: rtl يتعامل مع الاتجاه */}
          ...
          {/* بطاقات الفلاتر النشطة — برا لوحة الفلاتر فبتستحمل flex-row-reverse */}
          ...
          <Tabs defaultValue="stories" className="w-full">
            <div className={cn("flex", isRtl && "justify-end")}>
              <TabsList className="bg-card border-2 border-border/50 h-16 p-2 rounded-full mb-8 inline-flex" style={{ direction: isRtl ? 'rtl' : 'ltr' }}>
                {/* ✅ بدون flex-row-reverse — direction: rtl على TabsList يكفي */}
                <TabsTrigger
                  value="stories"
                  className="rounded-full px-8 font-black text-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all flex items-center gap-2"
                >
                ...
                <TabsTrigger
                  value="podcasts"
                  className="rounded-full px-8 font-black text-lg data-[state=active]:bg-secondary data-[state=active]:text-secondary-foreground transition-all flex items-center gap-2"
                >
            ...
            <TabsContent value="stories" className="mt-0 outline-none">
              {loadingStories ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                  ...
                  <h3 className="text-2xl font-black text-foreground">{t('لا توجد قصص', 'No stories found')}</h3>
                  <p className="text-muted-foreground font-bold max-w-md">
                    {t('لم نجد قصصاً تطابق الفلاتر المحددة. جرب تغيير التصنيف أو الفئة العمرية.', 'No stories match the selected filters. Try changing the category or age group.')}
                  </p>
                  {hasActiveFilters && (
                    <button onClick={clearFilters} className="mt-2 px-6 py-3 bg-primary text-primary-foreground rounded-full font-bold text-sm hover:bg-primary/90 transition-colors shadow-md">
                      {t('مسح الفلاتر', 'Clear filters')}
                    </button>
                  )}
                </div>
            ...
            <TabsContent value="podcasts" className="mt-0 outline-none">
              {loadingPodcasts ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
                  <div className="text-6xl">🎧</div>
                  <h3 className="text-2xl font-black text-foreground">{t('لا يوجد بودكاست', 'No podcasts found')}</h3>
                  <p className="text-muted-foreground font-bold max-w-md">
                    {t('لم نجد بودكاست يطابق الفلاتر المحددة. جرب تغيير التصنيف.', 'No podcasts match the selected filters. Try changing the category.')}
                  </p>
                  {hasActiveFilters && (
                    <button onClick={clearFilters} className="mt-2 px-6 py-3 bg-secondary text-secondary-foreground rounded-full font-bold text-sm hover:bg-secondary/90 transition-colors shadow-md">
                      {t('مسح الفلاتر', 'Clear filters')}
                    </button>
                  )}
                </div>
